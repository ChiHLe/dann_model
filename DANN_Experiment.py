...
for i in range(0,30):
    ...
    Model = Sequential()
    Model.add(Dense(50, activation='relu', input_shape=(max_words,)))
    ...
    for epoch in range(0,100):
        ...
        if epoch % 5 == 0:
            ...
            print("Domain Accuracy: %.2f%%" % (scores[1] * 100))
            print("Current maximum accuracy: %.2f%%" % max_score)
        #domain.fit(X_domain, y_domain, shuffle=True, epochs=1, batch_size=32, verbose=1)
    #Model.fit(X_train, [y_train, wrong_domain], shuffle=True,epochs=3, batch_size=5,verbose=2)
